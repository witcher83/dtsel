( function () {
	...
	function DTBox( elem, settings ) {
		...
		this.weekdays     = settings.localization.weekdays;
		this.months       = settings.localization.months;
		this.months_short = settings.localization.months_short;
		...
		Object.defineProperties( this, {
			...
			month_short: {
				get: function () {
					return self.months_short[self.month];
				},
			},
			state      : {
				get: function () {
		...
		if ( "DAYS" === this.bodyType ) {
			...
			if ( ! toAppend ) {
				...
				for ( var i = 0; i < 7; i++ ) {
					...
					cell.innerText = this.weekdays[i];
					cell.onclick   = null;
				}
			...
			if ( ! toAppend ) {
				...
				for ( var i = 0; i < 3; i++ ) {
					for ( var j = 0; j < 4; j++ ) {
						var monthShort                             = this.months_short[4 * i + j];
